...
def test_comment(client, auth):
    ...
    response = client.post(
        '/comment',
        data={'rating': 5, 'comment': 'Who needs quarantine?', 'movie_id': 2}
    )
    assert response.headers['Location'] == 'http://localhost/movies_by_year?date=2012&view_comments_for=2'
...
def test_movies_with_genre(client):
    ...
    # Check that all articles genreged with 'Health' are included on the page.
